...
async def periodic_backup_loop(interval_seconds: int = 120):
    """Safety-net backup: periodically pushes the current database file to a
    dedicated GitHub branch, so data survives Render's ephemeral filesystem
    across redeploys, in addition to the immediate backups triggered on writes."""
    while True:
        await asyncio.sleep(interval_seconds)
...
async def main():
    ...
    await github_backup.restore_from_github(DB_PATH)
    await init_db()
